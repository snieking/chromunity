...
} from "../../../blockchain/NotificationService";
...
> = props => {
  ...
  const [
    couldExistOlderNotifications,
    setCouldExistOlderNotifications
  ...
  function retrieveNotifications() {
    ...
    const loggedInUser = getAuthorizedUser();
    ...
    getUserNotificationsPriorToTimestamp(
      ...
      .then(retrievedNotifications => {
        setNotifications(
          Array.from(new Set(notifications.concat(retrievedNotifications)))
        );
        setLoading(false);
        setCouldExistOlderNotifications(
          retrievedNotifications.length >= notificationsPageSize
        );

        if (loggedInUser != null && loggedInUser.name === userId) {
          markNotificationsRead(getAuthorizedUser());
        }
      })
